...
### 一键部署
```bash
git clone https://github.com/noxenys/team-manage.git
cd team-manage
cp .env.example .env
docker compose up -d
```
...
| 变量名 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `APP_NAME` | 否 | "GPT Team 管理系统" | 应用名称，显示在网页标题和头部 |
| `APP_VERSION` | 否 | "0.1.0" | 应用版本号 |
| `APP_HOST` | 否 | "0.0.0.0" | 监听地址 |
| `APP_PORT` | 否 | 8008 | 监听端口 |
| `DEBUG` | 否 | False | 是否开启调试模式（生产环境建议 False） |
| `DATABASE_URL` | 否 | sqlite... | 数据库连接地址，支持 SQLite, PostgreSQL, MySQL 等 |
| `SECRET_KEY` | 是 | "change-me..." | Session 加密密钥（生产环境必须修改） |
| `ADMIN_PASSWORD` | 是 | "change-me" | 初始管理员密码（首次登录后请修改） |
| `LOG_LEVEL` | 否 | "INFO" | 日志级别 (DEBUG, INFO, WARNING, ERROR) |
| `PROXY_ENABLED` | 否 | False | 是否开启全局代理 |
| `PROXY` | 否 | "" | 代理地址（HTTP/SOCKS5） |
| `JWT_VERIFY_SIGNATURE` | 否 | False | 是否验证 JWT 签名（生产环境建议 True） |
| `TIMEZONE` | 否 | "Asia/Shanghai" | 系统时区设置 |
| `SYNC_ENABLED` | 否 | True | 是否开启定时同步 |
| `SYNC_INTERVAL_MINUTES` | 否 | 60 | 同步间隔（分钟） |
| `SYNC_RETRY_COUNT` | 否 | 1 | 失败重试次数 |
| `SYNC_RETRY_DELAY_SECONDS` | 否 | 5 | 失败重试等待秒数 |
...
### 3. 数据持久化
Docker 配置中已将宿主机的 `team_manage.db` 文件映射到容器内部，因此数据会保存在项目根目录下，容器删除后数据依然保留。
...
### 代理配置
如需通过代理访问 ChatGPT API，可在管理员面板的"系统设置"中配置代理：
...
### 用户兑换流程
1. **访问兑换页面**: http://localhost:8008/
2. **输入信息**: 填写邮箱地址和兑换码
3. **选择 Team**: 手动选择或自动分配 Team
4. **完成兑换**: 系统自动发送邀请到邮箱
5. **接受邀请**: 邮箱中点击邀请链接
...
- `POST /warranty/check` - 质保查询
- `GET /admin` - 管理员控制台
- `GET /admin/teams/import` - Team 导入页面
- `GET /admin/codes` - 兑换码列表
- `GET /admin/records` - 使用记录
- `GET /admin/teams/export` - 导出 Team（CSV/Excel）
- `GET /admin/codes/export` - 导出兑换码（CSV/Excel）
- `GET /admin/records/export` - 导出使用记录（CSV/Excel）
- `GET /admin/audit-logs` - 审计日志

## 💾 数据备份与恢复

本系统默认使用 SQLite 数据库，数据文件位于项目根目录下的 `team_manage.db`。

### 备份
直接复制 `team_manage.db` 文件到安全位置即可。
建议定期备份，特别是在进行版本更新或大规模数据操作前。

### 恢复
将备份的 `team_manage.db` 文件覆盖回项目根目录，并重启服务。
...
## 致谢
本项目基于 [tibbar213/team-manage](https://github.com/tibbar213/team-manage) 开发，感谢原作者的开源贡献。
